...
function getGradeKey(label: string): string {
  ...
  if (label.includes("الأولى اعدادي") || label.includes("الاولى اعدادي")) return "1AC";
  if (label.includes("السادس")) return "6AEP";
  if (label.includes("الخامس")) return "5AEP";
  if (label.includes("الرابع")) return "4AEP";
  if (label.includes("الثالث ابتدائي") || label.includes("الثالث")) return "3AEP";
  if (label.includes("الثاني")) return "2AEP";
  if (label.includes("الأول ابتدائي") || label.includes("الاول")) return "1AEP";
  return label;
}
...
function guessLanguageInfo(urlStr: string, textStr: string): { language: string, track: string } {
  ...
  if (norm.match(/\b(fr|francais|فرنسية|biof|international)\b/) || norm.includes("فرنسي")) {
    language = "fr";
    track = "BIOF";
  }
...
async function fetchHtml(url: string, retries = 3): Promise<string | null> {
  ...
  while (attempt < retries) {
    attempt++;
    console.log(`[CRAWL] Fetching: ${url} (Attempt ${attempt}/${retries})`);
    try {
      ...
      // 1-2 sec delay to not hammer the site
      await sleep(Math.floor(Math.random() * 1000) + 1000); 
      return res.data;
    } catch (err: any) {
      console.error(`[ERROR] Attempt ${attempt} failed to fetch ${url}: ${err.message}`);
      if (attempt >= retries || (err.response && err.response.status === 404)) {
        await sleep(1000);
        return null;
      }
...
function isValidMoutamadrisUrl(url: string): boolean {
  try {
    const parsed = new URL(url);
    if (parsed.hostname.includes("moutamadris.ma")) {
      const urlLower = url.toLowerCase();
      // If it is a PDF or an upload, allow it. Otherwise reject if it lacks '/cours/' prefix
      if (urlLower.endsWith(".pdf") || urlLower.includes("/wp-content/uploads/")) {
        return true;
      }
      return urlLower.includes("/cours/");
    }
  } catch {
    return false;
  }
...
// Utility to clean elements of headers/footers/navigators in Cheerio
function cleanNavigationAndFooters($: cheerio.CheerioAPI) {
  // Select standard navigation, headers, footer structures and discard them
...
export async function runMoutamadrisCrawl(outputJsonPath: string, outputCsvPath: string) {
  visitedUrls.clear(); // reset state
  ...
  function saveOutput() {
    ...
    console.log(`[SAVED] ${data.length} resources saved.`);
  }

  console.log("Starting Advanced Moutamadris Crawler...");
  
  const html = await fetchHtml(START_URL);
  if (!html) return;
  
  const $ = cheerio.load(html);
  // Just clean menu and sidebar on landing page to see links correctly
  ...
  $('a').each((_, el) => {
    ...
    if (href && TARGET_GRADES.some(tg => text.includes(tg))) {
      try {
        const fullUrl = new URL(href, START_URL).href;
        if (isValidMoutamadrisUrl(fullUrl)) {
          gradeLinks.push({ href: fullUrl, label: text });
        }
      } catch (e) {}
    }
  });
  ...
  for (const grade of gradeLinks) {
    ...
    $g('a').each((_, el) => {
      ...
      if (href && href.includes('moutamadris.ma') && text.length > 3) {
        if (!isUrlIgnored(href) && href !== grade.href && !href.includes('/category/')) {
           try {
             const fullUrl = new URL(href, grade.href).href;
             if (isValidMoutamadrisUrl(fullUrl)) {
               subjectLinks.push({ href: fullUrl, label: text });
             }
           } catch(e) {}
        }
      }
    ...
    for (const subject of subjectLinks) {
       ...
       $s('a').each((_, el) => {
          ...
          if (href && !isUrlIgnored(href)) {
             if (href.includes("moutamadris.ma") && href !== subject.href) {
                try {
                  const fullUrl = new URL(href, subject.href).href;
                  if (isValidMoutamadrisUrl(fullUrl)) {
                    lessonLinks.push({ href: fullUrl, label: text });
                  }
                } catch(e) {}
             }
          }
       ...
       for (const lesson of lessonLinks) {
          ...
          $l('a').each((_, el) => {
             ...
             try {
                ...
                if (isPdf || hasArabicKeywords) {
                   ...
                   const resType = guessResourceType(contextText + " " + lesson.label);
                   
                   // Ignore exam-only pages unless resource_type = exam
                   const isExamPage = lesson.href.includes("/examens/") || lesson.href.includes("/forod/") || lesson.href.includes("/controle/");
                   if (isExamPage && resType !== "exam" && resType !== "assessment") {
                      return; 
                   }
          ...
          if (resources.size > 0 && resources.size % 20 === 0) {
             saveOutput();
          }
       }
    }
